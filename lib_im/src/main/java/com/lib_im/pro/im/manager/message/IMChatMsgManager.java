...
import com.lib_im.pro.entity.SessionItem;
...
public class IMChatMsgManager implements ChatMsgManager<ChatMessage>,
        ChatManagerListener, StanzaListener, ReceiptReceivedListener, org.jivesoftware.smack.MessageListener {
                                ...
                                if (message.getBodies().size() > 0) {//如果是空消息则不接收
                                    ...
                                    if (messageBody != null) {
                                        ...
                                        if (LiteChat.chatClient.getSessionManager() != null) {
                                            LiteChat.chatClient.getSessionManager().postSession(SessionItem.toSessionItem(chatMessage), count);
                                        }
                                        if (count > 0) {
                                            if (LiteChat.chatClient.getNotifyManager() != null) {
                                                LiteChat.chatClient.getNotifyManager().playChatMessage(false, chatMessage.getFromId(), chatMessage.getFromName(), "");
                                            }
                                        }
                                        //------END------------------------------------------
...
//                //**发送消息成功，创建session-------------------start*/
                    ...
                    //--通知SessionManager--创建更新Sessioin-----------------
                    if (LiteChat.chatClient.getSessionManager() != null) {
                        LiteChat.chatClient.getSessionManager().postSession(SessionItem.toSessionItem(msg), 0);
                    }
                    if (_call != null) {
                        _call.onSuccess(msg);
                    }
     ...
     */
    @Override
    public void sendRoomMessage(String chatRoomJid, ChatMessage chatMessage, MessageCallBack _call) {
     ...
     */
    @Override
    public void getRoomHistoryMessage(String groupId,String logId, String messageId,int page ,int rows) {
        final List<ChatMessage> _historyList = new ArrayList<>();
        //TODO 此处进行网络请求，请求返回数据操作如下注释所示，此处逻辑根据后台群聊插件而定
        LiteChat.imRequestManager.getListInstance().queryGroupChatRecord(groupId,logId,messageId,page,rows).subscribe(new SimpleListObserver<GroupChatRecord>() {
            @Override
                ...
                if (groupChatRecords != null) {
                    for (GroupChatRecord groupChatRecord : groupChatRecords) {
                        if (groupChatRecord != null) {
                            ChatMessage chatMessage = packRoomHistoryMessage(groupChatRecord);
...
//    }
    ...
    public void processMessage(Message message) {
        if (message.getBodies().size() > 0) {
            ...
            /**添加群组历史消息的判断*/
            final String _userId = LiteChat.chatClient.getConfig(ChatCode.KEY_USER_ID);
            final String _userName = LiteChat.chatClient.getConfig(ChatCode.KEY_USER_NICK_NAME);
            String _fromUserName = strFrom.split("/")[1];
            /**解析接收到的消息json*/
            try {
                if (!_fromUserName.equals(_userName)) {//如果发送消息的人与当前接收消息的人一致则过滤掉此消息
                    JSONObject jsonObject = new JSONObject(message.getBody());
                    String sendUserId = jsonObject.getString("sendUserId");
                    String sendUserName = jsonObject.getString("sendUserName");
                    String sendUserIcon = jsonObject.getString("headIcon");
                    ...
                    GroupContact groupContact = groupContactManager.getGroupContact(chatMessage.getRoomId());
                    String roomName = groupContact.getGroupName();
                    if (count > 0) {
                        if (LiteChat.chatClient.getNotifyManager() != null) {
                            if (roomName != null && !roomName.equals("")) {
                                LiteChat.chatClient.getNotifyManager().playChatMessage(true, groupContact.getGroupID(), roomName, groupContact.getGroupJid());
                            }
                        }
                    }
                    //------END------------------------------------------
     ...
     */
    @Override
    public void getChatHistoryMessage(String messageId, String fromUserId, String toUserId, int page,int rows) {
        final List<ChatMessage> _chatMsgList = new ArrayList<>();
        //TODO 获取单个人聊天历史记录，处理逻辑如下注释，这里取决于后台单聊历史纪录插件的逻辑
        LiteChat.imRequestManager.getListInstance().queryChatRecord(messageId, fromUserId, toUserId, page,rows).subscribe(new SimpleListObserver<ChatRecord>() {
            @Override
    ...
    public void getRoomUnReadMessageCount(String string) {
        ...
        LiteChat.imRequestManager.getListInstance().queryGroupRecordCount(string).subscribe(new SimpleListObserver<GroupChatRecord>() {
            @Override
            public void onNext(@NonNull List<GroupChatRecord> groupChatRecords) {
                if (groupChatRecords != null) {
                    for (GroupChatRecord groupChatRecord : groupChatRecords) {
                        if (groupChatRecord != null) {
                            int unReadCount = Integer.parseInt(groupChatRecord.getCount());
                            if (unReadCount > 0) {//有未读消息时创建session
                                ChatMessage chatMessage = packRoomHistoryMessage(groupChatRecord);
                                LiteChat.chatCache.saveObject(chatMessage.getRoomId(), chatMessage);
                                //--通知SessionManager--创建更新Sessioin-----------------
                                if (LiteChat.chatClient.getSessionManager() != null) {
                                    LiteChat.chatClient.getSessionManager().postSession(SessionItem.toSessionItem(chatMessage), unReadCount);
                                }
                                GroupContactManager<GroupContact> groupContactManager = LiteChat.chatClient.getGroupContactManager();
                                GroupContact groupContact = groupContactManager.getGroupContact(chatMessage.getRoomId());
                                String roomName = groupContact.getGroupName();
     ...
     * @descript 打包群消息实体
     */
    private void packRoomMessageEntity(final ChatMessage chatMessage, String roomId, String _selfId, String _selfName) {
...
//                            mTable.save(chatMessage);
     ...
     * @descript 包装单人聊天相关实体
     */
    public void packSigleMessageEntity(final ChatMessage chatMessage, String _userName, String _userId) {
        /**构建消息实体-------------------------------------------------------------------------------start*/
        chatMessage.setMT(Boolean.TRUE);
